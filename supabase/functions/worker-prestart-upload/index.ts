...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (!/^REQ-[A-Z0-9-]{4,80}$/.test(requestNo) || token.length < 32 || token.length > 128) {
    ...
    if (!extensions[mimeType] || !Number.isSafeInteger(fileSize) || fileSize < 1 || fileSize > maxFileBytes) {
    ...
    const { data: task, error: taskError } = await admin.rpc("read_task_with_token", {
      p_request_no: requestNo,
      p_token: token,
      p_role: "worker"
    });
    if (taskError || !task) return json({ error: "TASK_LINK_EXPIRED_OR_INVALID" }, 403, origin);

    if (task.status !== "arrived" || task.current_actor !== "worker" || task.next_action !== "prestart_confirmation") {
      return json({ error: "TASK_NOT_READY_FOR_PRESTART" }, 409, origin);
    }

    const { data: taskRow, error: taskRowError } = await admin
      .from("service_requests")
      .select("assigned_worker_phone")
      .eq("request_no", requestNo)
      .single();

    const workerPhone = String(taskRow?.assigned_worker_phone || "").replace(/\D/g, "");
    if (taskRowError || !workerPhone) return json({ error: "WORKER_PHONE_REQUIRED" }, 409, origin);

    const path = `${requestNo}/${workerPhone}/prestart-photos/${Date.now()}-${crypto.randomUUID()}.${extensions[mimeType]}`;
